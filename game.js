...
function Game(
  ...
  this.sprite = new Button(
    ...
    this._spriteSheet.getSrcRect("bigButton"),
    new Vector2(50, 50),
    "test",
    1
  );
  ...
  _resizeHandler: {
    ...
    value: function () {
      ...
      if (newAspectRatio > nativeAspectRatio) {
        ...
        newHeight = Help.clamp(
          newWidth / nativeAspectRatio,
          minHeight,
          maxHeight
        );
      }

      this._canvas.style.width = newWidth + "px"; //Ny höjd och bredd uträknad.
      this._canvas.style.height = newHeight + "px";
      this._canvas.style.marginTop =
        Help.clampMin((window.innerHeight - newHeight) / 2, 0) + "px";
      this._canvas.style.marginLeft =
        Help.clampMin((window.innerWidth - newWidth) / 2, 0) + "px";
      this._canvas.style.marginBottom =
        (window.innerHeight - newHeight) / 2 + "px";
      this._canvas.style.marginRight =
