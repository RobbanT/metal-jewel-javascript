...
function Game(divContainer, canvas, nativeWidth, nativeHeight, minScale, maxScale, imagePath) {
    ...
    this._scaleWidth = undefined;
    this._scaleHeight = undefined;
    this._mouse = new Mouse();
    this._keyboard = new Keyboard();
    this._delta = 1 / 60;
    this._spriteSheet = new SpriteSheet(imagePath);
...
Object.defineProperties(Game.prototype, {
    _resizeHandler: {
        enumerable: true,
        value: function(){
            var minWidth = this._nativeWidth * this._minScale;
            var minHeight = this._nativeHeight * this._minScale;
            var maxWidth = this._nativeWidth * this._maxScale;
            var maxHeight = this._nativeHeight * this._maxScale;
            var nativeAspectRatio = this._nativeWidth / this._nativeHeight; //Spelets ursprungliga aspectratio.
            var newWidth = window.innerWidth;
            var newHeight = window.innerHeight;
            var newAspectRatio = newWidth / newHeight;  //Webbläsarens aspectratio.

            if (newAspectRatio > nativeAspectRatio) {   //Webbläsaren är för bred (webbläsarens aspectratio är större).
                newWidth = Help.clamp(newHeight * nativeAspectRatio, minWidth, maxWidth);
                newHeight = Help.clamp(newHeight, minHeight, maxHeight);
            }else{  //Webbläsaren är för hög (webbläsarens aspectratio är mindre).
                newWidth = Help.clamp(newWidth, minWidth, maxWidth);
                newHeight = Help.clamp(newWidth / nativeAspectRatio, minHeight, maxHeight);
            }

            this._divContainer.style.width = newWidth + "px";    //Ny höjd och bredd uträknad.
            this._divContainer.style.height = newHeight + "px";
            this._divContainer.style.marginTop = Help.clampMin((window.innerHeight - newHeight) / 2, 0) + "px";
            this._divContainer.style.marginLeft = Help.clampMin((window.innerWidth - newWidth) / 2, 0) + "px";
            this._divContainer.style.marginBottom = (window.innerHeight - newHeight) / 2 + "px";
            this._divContainer.style.marginRight = (window.innerWidth - newWidth) / 2 + "px";
            this._canvas.width = newWidth;
            this._canvas.height = newHeight;
            this._scaleWidth = this._canvas.width / this._nativeWidth;
            this._scaleHeight = this._canvas.height / this._nativeHeight;
        }
    },
    _initSpriteSheet: {
        enumerable: true,
        value: function(spriteSheet){
    ...
    update: {
        enumerable: true,
        value: function(){
            this._mouse.update(this._divContainer, this._scaleWidth, this._scaleHeight);
